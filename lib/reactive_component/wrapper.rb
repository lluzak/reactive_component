...
module ReactiveComponent
  module Wrapper
    ...
    def wrap(component_class, record, inner_html, stream: nil, client_state: nil, strategy: nil, component_name: nil,
      ...
      # arrives, and a presence field re-renders before there is any broadcast
      # at all. A notify one asks the server for its data instead, so the
      # attribute would be the page a second time, both branches of every
      # extracted expression included.
      if (client_state || presence_fields.any?) && strategy.to_s != 'notify'
