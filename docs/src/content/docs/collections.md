...
- **An item ships as its extracted expressions only** — never the raw record (that would leak every column). Every read of the loop variable must therefore be something the compiler can evaluate per item; printing the item itself (`<%= subtask %>`) raises `ReactiveComponent::CompileError` at compile time. Large collections mean large payloads: consider pagination.
