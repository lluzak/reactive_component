...
**Your own broadcast:** the action's model change broadcasts to every viewer, including the one who clicked. Set [`ReactiveComponent.skip_own_broadcasts`](/reactive_component/configuration/#reactivecomponentskip_own_broadcasts) to have components on the acting page ignore it and keep the optimistic update.
