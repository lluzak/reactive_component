...
// The cable registry is mocked so a test can see what the controller performed
// without opening a socket. Stimulus itself is real: it comes in as a peer
// dependency, and its Controller supplies the element off the context.
const performed = []
let connected = true
...
function build({ cursors = false, autoConnect = true } = {}) {
  ...
  const controller = new PresenceController({ scope: { element } })
  ...
  Object.entries({
    streamValue: "signed-stream",
    hasStreamValue: true,
    ttlValue: 90000,
    cursorsValue: cursors
  }).forEach(([name, value]) => Object.defineProperty(controller, name, { value, writable: true }))
  controller.connect()
